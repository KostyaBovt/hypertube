...
get_profile(Data0, #session{uid = MyUId, uname = Uname}) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"uid">> := <<"me">>}} ->
            {ok, Profile} = hyper_db:get_user(MyUId),
            {ok, hyper_lib:conv_locaction(add_photos(Profile, MyUId))};
        {ok, #{<<"uid">> := UId0}} ->
            UId1 = binary_to_integer(UId0),
            case hyper_db:get_user(UId1, MyUId) of
                {ok, Profile0} ->
                    Profile1 = Profile0#{<<"status">> => hyper_lib:is_subscribed(UId1)},
                    true = hyper_db:add_visit(UId1, MyUId),
                    hyper_lib:send_notification(UId1, #{type => visits, text => <<"Your profile visited by ", Uname/binary>>}),
...
get_visit_history(#session{uid = MyUId}) ->
    {ok, Res} = hyper_db:get_visit_history(MyUId),
    {ok, add_photo(Res)}.

get_likes(#session{uid = MyUId}) ->
    {ok, Res} = hyper_db:get_likes(MyUId),
    {ok, add_photo(Res)}.

get_chats(#session{uid = MyUId}) ->
    {ok, Res} = hyper_db:get_chats(MyUId),
    {ok, add_photo(Res)}.
...
get_chat_history(Data0, #session{uid = MyUId}) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"chat_id">> := ChatId}} ->
            case hyper_db:is_chat_member(ChatId, MyUId) of
                true ->
                    io:format("CHAT ID: ~p~n", [ChatId]),
                    {ok, _} = hyper_db:get_chat_messages(ChatId);
    ...
    case liver:validate(VSchema, Data0) of
	    ...
	    Location = {Lat, Lng},
            case update_profile(MyUId, Gender, SPref, Bio, Interests, Avatar, P1, P2, P3, P4, Fname, Lname, Uname,
                ...
                {ok, P} ->
                    hyper_mnesia:update_session(S, Uname, Interests, Location),
                    {ok, hyper_lib:conv_locaction(add_photos(P, MyUId))};
                E -> E
            end;
        E -> E
    end.
...
update_profile(UID, Gender, SPref, Bio, Interests, Avatar, P1, P2, P3, P4, Fname, Lname, Uname, Birthdate, Location) ->
    ...
    case lists:foldl(fun({error, L}, Acc) -> Acc#{L => <<"invalid photo">>};
        ...
        _ ->
            ...
            {ok, _} = hyper_db:update_profile(UID, Gender, SPref, Bio, Interests,
...
update_photo(UID, PhotoBin, Label) ->
    Name = <<(hyper_lib:md5(PhotoBin))/binary, ".jpeg">>,
    Path = <<"priv/static/photos/", Name/binary>>,
    case filelib:is_regular(Path) of
        true -> ok;
        false -> file:write_file(Path, PhotoBin)
    end,
    true = hyper_db:update_photo(UID, Name, Label).
...
update_password(Data0, #session{uid = UId}) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"old_pass">> := OldPass, <<"new_pass">> := NewPass}} ->
            {ok, #{<<"password">> := PassHash}} = hyper_db:get_user_password(UId),
...
update_email(Data0, #session{uname = Uname, uid = UId}, BaseUrl) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"email">> := Email}} ->
            ...
            <<_/binary>> = hyper_lib:send_email(<<"hyper -- email changing">>, iolist_to_binary(Body), Email),
            ok;
        E -> E
    end.
...
like(Data, #session{uid = MyUId, uname = MyUname}) ->
    ...
    case liver:validate(VSchema, Data) of
        {ok, #{<<"uid">> := UId0}} ->
            UId1 = binary_to_integer(UId0),
            true = hyper_db:create_like(MyUId, UId1),
            case hyper_db:like_exists(UId1, MyUId) of
                true ->
                    true = hyper_db:create_chat(UId1, MyUId),
                    Msg = <<"You liked back, now you can chat with ", MyUname/binary>>,
                    hyper_lib:send_notification(UId1, #{type => chats, text => Msg});
                false ->
                    Msg = <<"Your liked by ", MyUname/binary>>,
                    hyper_lib:send_notification(UId1, #{type => likes, text => Msg})
            end;
        E -> E
    end.
...
unlike(Data, #session{uid = MyUId, uname = MyUname}) ->
    ...
    case liver:validate(VSchema, Data) of
        {ok, #{<<"uid">> := UId0}} ->
            UId1 = binary_to_integer(UId0),
            hyper_db:delete_like(MyUId, UId1),
            Msg = <<"User ", MyUname/binary, " unliked you">>,
            case hyper_db:like_exists(UId1, MyUId) of
                true ->
                    hyper_lib:send_notification(UId1, #{type => chats, text => Msg}),
                    true = hyper_db:delete_chat(UId1, MyUId),
                    ok;
                false ->
                    hyper_lib:send_notification(UId1, #{type => likes, text => Msg}),
                    ok
...
rate(Data0, #session{uid = MyUId}) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"rating">> := Rating, <<"uid">> := UId0}} when Rating >= 0 andalso Rating =< 10 ->
            UId1 = binary_to_integer(UId0),
            case hyper_db:get_user(UId1, MyUId) of
                {ok, _} ->  {ok, _} = hyper_db:set_rating(MyUId, UId1, Rating);
...
send(Data0, #session{uid = MyUId, uname = MyUname}) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"chat_id">> := ChatId, <<"message">> := Msg}} ->
            case hyper_db:get_potential_lover(ChatId, MyUId) of
                {ok, #{<<"user_id">> := UId}} ->
                    {ok, M} = Res = hyper_db:save_message(ChatId, MyUId, MyUname, Msg),
                    hyper_lib:send_notification(UId, M#{type => chats}),
                    Res;
...
block(Data0, #session{uid = MyUid}) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"uid">> := UId0}} ->
            true = hyper_db:block_user(MyUid, binary_to_integer(UId0)),
            ok;
        E -> E
    end.
...
unblock(Data0, #session{uid = MyUid}) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"uid">> := UId0}} ->
            true = hyper_db:unblock_user(MyUid, binary_to_integer(UId0)),
            ok;
        E -> E
    end.
...
report(Data0, #session{uid = MyUid}) ->
    ...
    case liver:validate(VSchema, Data0) of
        {ok, #{<<"uid">> := UId0}} ->
            true = hyper_db:report_user(MyUid, binary_to_integer(UId0)),
            ok;
        E -> E
    end.
